...
# -----------------------------
# Part 4: Setup I — McNemar's test (Exercise 7)
# -----------------------------
def mcnemar(y_true: np.ndarray, yhatA: np.ndarray, yhatB: np.ndarray, alpha: float = 0.05) -> Tuple[float, Tuple[float, float], float]:
    """
    CITATION:
      - (Exercise 7, Part 2) McNemar’s test function and exact binomial p-value:
        p = 2 * st.binom.cdf(min(n12, n21), n=n12+n21, p=0.5)
      - (Exercise 7) Confidence interval mapping via Jeffreys prior:
        CI = tuple(bound*2 - 1 for bound in st.beta.interval(1-alpha, a=f, b=g))
    CHANGES:
      - Wrapped into a function here; computes theta_hat = (n12 - n21)/(n12+n21).
      - Uses Jeffreys prior (0.5, 0.5): f = n12 + 0.5, g = n21 + 0.5.
    """
    import scipy.stats as st

    # Contingency counts
    correctA = (yhatA == y_true)
    correctB = (yhatB == y_true)

    n12 = int(np.sum((~correctA) & (correctB)))  # A wrong, B correct
    n21 = int(np.sum((correctA) & (~correctB)))  # A correct, B wrong

    # Point estimate (difference in accuracy)
    n_discord = n12 + n21
    theta_hat = 0.0 if n_discord == 0 else float(n12 - n21) / float(n_discord)

    # Exact two-sided binomial p-value (Exercise 7)
    p = 1.0
    if n_discord > 0:
        p = 2.0 * st.binom.cdf(min(n12, n21), n=n_discord, p=0.5)
        p = float(min(1.0, p))

    # CI via Jeffreys prior mapped to theta in [-1, 1] (Exercise 7)
    f = n12 + 0.5
    g = n21 + 0.5
    CI_beta = st.beta.interval(1 - alpha, a=f, b=g)
    CI = (float(CI_beta[0] * 2.0 - 1.0), float(CI_beta[1] * 2.0 - 1.0))

    return theta_hat, CI, p


def run_setupI_mcnemar(
    X_df: pd.DataFrame,
    y: np.ndarray,
    seed: int = 1234,
    n_epochs_ann: int = 500,
    lr_ann: float = 0.01,
) -> Dict[str, Any]:
    """
    Perform Setup I (single fixed train/test split) and apply McNemar’s test pairwise.

    CITATION:
      - (Exercise 7) Setup I (fixed training set) + McNemar test function and usage.
      - Uses the same preprocessing and model code blocks cited earlier (Exercises 2,4,6,8,9).
    CHANGES:
      - Use a single StratifiedKFold split (K=2, one split used) to form one fixed hold-out.
      - Tune logistic λ and ANN H on the training part only (inner CV), then predict on the fixed test part.
    """
    # One fixed split (Setup I)
    skf = StratifiedKFold(n_splits=2, shuffle=True, random_state=seed)
    (tr_idx, te_idx) = next(iter(skf.split(X_df, y)))
    X_tr_df, X_te_df = X_df.iloc[tr_idx], X_df.iloc[te_idx]
    y_tr, y_te = y[tr_idx], y[te_idx]

    numeric_cols, categorical_cols = split_feature_types(X_tr_df)

    # Hyperparameter grids (same as nested CV usage above)
    lambdas = np.logspace(-4, 2, 15)
    hidden_grid = [1, 2, 4, 8, 16, 32]

    # Baseline (train-majority)
    yhat_base = majority_class_predictor(y_tr, len(y_te))

    # Logistic: tune λ on training part
    best_lam, _ = tune_logistic_lambda_inner_cv(
        X_tr_df, y_tr, numeric_cols, categorical_cols, lambdas, K_inner=10, seed=seed
    )
    # Fit logistic on train and predict on test
    ...
    yhat_log = clf_log.predict(X_te_log)

    # ANN: tune H on training part
    best_H = tune_ann_hidden_inner_cv(
        X_tr_df, y_tr, numeric_cols, categorical_cols, hidden_grid, K_inner=10, seed=seed, n_epochs=n_epochs_ann, lr=lr_ann
    )
    ...
    model_ann = train_ann_binary(X_tr_ann, y_tr, n_epochs=n_epochs_ann, lr=lr_ann, hidden_units=best_H, seed=seed)
    with torch.no_grad():
        yprob_ann = model_ann(torch.tensor(X_te_ann, dtype=torch.float32)).numpy().reshape(-1)
    yhat_ann = (yprob_ann >= 0.5).astype(int)

    # Pairwise McNemar tests
    pairs = {
        "ANN_vs_LOG": (yhat_ann, yhat_log),
        "ANN_vs_BASE": (yhat_ann, yhat_base),
        "LOG_vs_BASE": (yhat_log, yhat_base),
    }
    out = {}
    for name, (ya, yb) in pairs.items():
        theta_hat, CI, p = mcnemar(y_true=y_te, yhatA=ya, yhatB=yb, alpha=0.05)
        out[name] = {"theta_hat": theta_hat, "CI95": CI, "p_value": p}
        print(f"[Setup I - McNemar] {name}: theta={theta_hat:.4f}, CI=({CI[0]:.4f},{CI[1]:.4f}), p={p:.4g}")

    return {"results": out, "split_sizes": {"n_train": int(len(y_tr)), "n_test": int(len(y_te))}}
...
def main():
    ...
    # Part 4 (Setup I - McNemar’s test)
    print("\nPart 4) Setup I — McNemar’s test (Exercise 7, Part 2):")
    _ = run_setupI_mcnemar(X_df, y.values, seed=seed, n_epochs_ann=500, lr_ann=0.01)
